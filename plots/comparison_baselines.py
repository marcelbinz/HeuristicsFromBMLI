...
if args.recompute:
    ...
    save_path = 'data/logprobs' + str(args.study) + '.pth'
    ...
    for participant in tqdm(range(inputs_a.shape[0])):
        for m, model_class in enumerate(models):
            for task in range(inputs_a.shape[2]):
                ...
                logprobs[participant, m] = logprobs[participant, m] + predictive_distribution.log_prob(predictions[participant, :, [task]]).sum()
    ...
    load_path = 'data/logprobs' + str(args.study) + '.pth'
    ...
    if args.study == 3:
        ...
        ylabels = ['Guessing', 'Ideal Observer', 'Equal Weighting', 'Single Cue', 'BMLI']
    else:
    ...
    plt.tight_layout()
    fig.savefig('plots/figures/posterior' + str(args.study) + '.pdf', bbox_inches='tight')
    plt.show()
